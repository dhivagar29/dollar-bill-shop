...
export function CartIconClient({ cartLabel, initialCart }: CartIconClientProps) {
  ...
  return (
    <button
      onClick={openOverlay}
      className="flex cursor-pointer items-center justify-center gap-1.5 text-foreground transition-colors hover:text-primary"
      type="button"
    >
      <span className="relative">
        <HandbagIcon className="size-5" />
        {quantity > 0 && (
          <span className="absolute -top-2 -right-1 flex size-4 items-center justify-center rounded-full bg-primary text-xxs leading-none text-primary-foreground">
            {quantity}
          </span>
        )}
      </span>
